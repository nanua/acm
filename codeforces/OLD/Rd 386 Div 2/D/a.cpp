...
int main() {
	...
	while ((a[0] || a[1]) && a[0] != a[1]) {
		if (a[1] > a[0]) swap(a[1], a[0]), swap(t[0], t[1]);
		if (a[0] > k && a[1] == 0) puts("NO"), exit(0);
		
		for (int i = 1, j = min(k, abs(a[1]-a[0])); i <= j; ++i)
			ans[++sz] = t[0], a[0]--;
		if (a[1]) ans[++sz] = t[1], a[1]--; 		
	}

	for (int i = 1; i <= a[1]; ++i) ans[++sz] = t[0], ans[++sz] = t[1];
