#include <bits/stdc++.h>
using namespace std;

#define clr(x, y) memset(x, y, sizeof(x))
#define cpy(x, y) memcpy(x, y, sizeof(y))
#define all(x) x.begin(), x.end()
#define sqr(x) ((x) * (x))
#define sz(x) ((ll)x.size())

#define mk make_pair
#define pb push_back
#define pk pop_back
#define fi first
#define se second

#define add_edge(x, y) (to[++edg] = y, nt[edg] = la[x], la[x] = edg)
#define add_wedge(x, y, w) (to[++edg] = y, wgt[edg] = w, nt[edg] = la[x], la[x] = edg)
#define lowbit(x) ((x) & (-x))

typedef int lint;
typedef long long ll;
typedef long double ld;
typedef pair <lint, lint> pii;

const ld eps = 1e-12, pi = acos(-1);
const lint maxn = 2e5+50, maxm = 1e5+50, inf = (lint)0x3f3f3f3f, mod = (lint)1e9 + 7, sigma = 129;
const ll INF = (ll)1e17;

typedef lint arrn[maxn];
typedef lint arrm[maxm];


template<class Q> void gint(Q &ret, char ch = getchar()) {
	while (!isdigit(ch)) ch = getchar();
	for (ret = 0; isdigit(ch); ch = getchar()) ret = ret * 10 + ch - '0';
}
int n, a, b, k, sz, asz, tot;
arrn ans, g;
pii v[maxn];

int main() {
#ifdef LOCAL
	freopen("a.in","r",stdin);
	freopen("a.out","w",stdout);
#endif
	cin >> n >> a >> b >> k; //grid, n of ship, l of ship, shots

	for (int i = 1; i <= n; ++i) scanf("%1d", g+i);
	g[0] = g[n+1] = 1;

	for (int i = 1, j; i <= n && j <= n; ) {
		while (g[i] == 1 && i <= n) ++i;
		if (g[i] == 0) {
			for (j = i; g[j] == 0; ++j);
			if (j-i >= b) v[++sz] = pii(i, j-1), tot += (j-i) / b;
			i = j;
		}
	}

	for (int i = 1; i <= sz; ++i)
		while (tot >= a && (v[i].se - v[i].fi + 1 >= b))
			ans[++asz] = v[i].fi + b - 1, v[i].fi += b, tot--;

	cout << asz << endl;
	for (int i = 1; i <= asz; ++i) cout << ans[i] << " ";
}
 
