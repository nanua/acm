...
string dfs(string pre, string in) {
	...
	int pos = in.find(rt, 0);
	
	string left1 = pre.substr(1, pos), left2 = in.substr(0, pos);
	string right1 = pre.substr(pos+1, sz(in) - pos - 1), right2 = in.substr(pos+1, sz(in) - pos - 1);

	string left = dfs(left1, left2);
	string right = dfs(right1, right2);
